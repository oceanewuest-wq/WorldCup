...
async function loadPersistedState() {
  ...
  // Calculate prevRanks from existing data if not set
  if (!sharedState.prevRanks || Object.keys(sharedState.prevRanks).length === 0) {
    try {
      const players = Object.keys(sharedState.players || {});
      const matches = sharedState.matches || [];
      const scores = sharedState.scores || {};
      const bets = sharedState.bets || {};
      
      // Get all finished matches sorted by date
      const finished = matches
        .filter(m => scores[m.id]?.status === 'FINISHED')
        .sort((a, b) => new Date(a.date) - new Date(b.date));
      
      if (finished.length >= 2) {
        // Calculate points excluding the last 2 matches (to show movement)
        const prevFinished = finished.slice(0, -2);
        
        function getStageMultiplier(stage) {
          if (['LAST_32'].includes(stage)) return { exact: 9, winner: 4 };
          if (['LAST_16', 'QUARTER_FINALS'].includes(stage)) return { exact: 9, winner: 4 };
          if (['SEMI_FINALS', 'FINAL', 'THIRD_PLACE'].includes(stage)) return { exact: 11, winner: 5 };
          return { exact: 7, winner: 3 };
        }
        
        const prevPts = {};
        players.forEach(p => {
          let pts = 0, exact = 0;
          prevFinished.forEach(m => {
            const bet = bets[m.id]?.[p];
            const sc = scores[m.id];
            if (!bet || !sc || !sc.regular) return;
            const [bH, bA] = bet;
            const [aH, aA] = sc.regular;
            const mult = getStageMultiplier(m.stage);
            if (bH === aH && bA === aA) { pts += mult.exact; exact++; }
            else {
              const bW = bH > bA ? 'h' : bA > bH ? 'a' : 'd';
              const aW = aH > aA ? 'h' : aA > aH ? 'a' : 'd';
              if (bW === aW) pts += mult.winner;
            }
          });
          prevPts[p] = { pts, exact };
        });
        ...
        const prevSorted = players
          .sort((a, b) => (prevPts[b]?.pts || 0) - (prevPts[a]?.pts || 0) || (prevPts[b]?.exact || 0) - (prevPts[a]?.exact || 0));
        
        sharedState.prevRanks = {};
        prevSorted.forEach((p, i) => { sharedState.prevRanks[p] = i + 1; });
        console.log('prevRanks calculated from historical data:', sharedState.prevRanks);
      }
    } catch(e) { console.log('prevRanks calc error:', e.message); }
  }
}
...
async function syncMatches() {
  const now = Date.now();
  const hasTBD = (sharedState.matches||[]).some(m=>m.team1==='TBD'||m.team2==='TBD');
  const interval = hasTBD ? 10 * 60 * 1000 : 6 * 3600 * 1000;
  if (now - sharedState.lastMatchSync < interval) return;
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/api/state' && req.method === 'GET') {
    // Trigger match sync if TBD matches exist
    const hasTBD = (sharedState.matches||[]).some(m=>m.team1==='TBD'||m.team2==='TBD');
    if (hasTBD) syncMatches().catch(()=>{});
  ...
  if (req.url === '/api/admin' && req.method === 'POST') {
    ...
    if (action === 'placeBetForPlayer') {
      const { username, matchId, score } = data;
      if (!sharedState.bets[matchId]) sharedState.bets[matchId] = {};
      sharedState.bets[matchId][username] = score;
    }
    if (action === 'applyPenalty') {
      const { username, points } = data;
      if (!sharedState.penalties) sharedState.penalties = {};
      sharedState.penalties[username] = (sharedState.penalties[username] || 0) + points;
    }
